...
@dataclass(frozen=True)
class Verdict:
    """What one classifier said about one command."""
...
def load_corpus(path: Path) -> list[Row]:
    ...
    Accepts a bare list or an object wrapping one under ``golden_paths`` / ``rows``,
    because the seed is written by a sibling tool and a gate that hard-codes one of
    those spellings fails on a cosmetic change to the other.

    Every rejection is a :class:`DenyDiffError`. A corpus that silently drops a
    ...
    if isinstance(payload, dict):
        for key in ("golden_paths", "rows"):
            if key in payload:
                payload = payload[key]
                break
        else:
            raise DenyDiffError(
                f"corpus {path} is an object without a 'golden_paths' or 'rows' key"
            )
...
def _row(path: Path, position: int, entry: object) -> Row:
    ...
    # ``command`` is accepted alongside the seed's own ``command_or_flow`` so a
    # hand-written fixture does not have to spell the seed's schema to be usable.
    command = entry.get("command_or_flow", entry.get("command"))
...
def resolve_checkout(repo_root: Path, ref: str, dest: Path) -> Path:
    ...
    is what lets the exit-code paths be proven against a two-line fake classifier
    instead of against the real 17,000-line one, whose verdicts are the thing
    under test and cannot also be the fixture.
    """
...
def classify(checkout: Path, commands: list[str], *, home: Path) -> list[Verdict]:
    """Classify *commands* with the deny classifier living under *checkout*.

    One child for the whole list: the classifier's import cost dwarfs its
    per-command cost, so per-row spawning would make the gate slower than the test
    suite it guards without changing a single verdict.
    """
    if not commands:
        return []
    ...
    for entry in verdicts:
        ...
        out.append(Verdict(denied=bool(entry["denied"]), reason=str(entry.get("reason") or "")))
    return out


def _worker_main() -> int:
    """The child: import the product's classifier and report a verdict per command.
    ...
    # than assumed.
    # ``normcase`` before the containment test: Windows paths are case-insensitive
    # and accept either separator, so a raw comparison can report "outside" for the
    ...
    for command in commands:
        try:
            reason = security.is_denied(command)
        except Exception as exc:
            verdicts.append({"error": f"{type(exc).__name__} on {command!r}: {exc}"})
            continue
        verdicts.append({"denied": bool(reason), "reason": (reason or "")[:_REASON_CHARS]})
...
def render_text(report: Report) -> str:
    ...
    lines = [
        ...
        f"- platform `{report.platform}`, corpus `{report.corpus}`",
    ...
    if report.regressions:
        ...
        for row, verdict in report.regressions:
            ...
            lines.append(f"    - head refuses: {verdict.reason}")
        lines.append("")
    else:
    ...
    if report.loosenings:
        ...
        for row, verdict in report.loosenings:
            lines.append(f"- `{row.command}` _[{row.platform}]_")
            lines.append(f"    - base refused: {verdict.reason}")
        lines.append("")
...
def render_json(report: Report) -> str:
    """The machine rendering, for a caller that wants the rows rather than the prose."""
    return json.dumps(
        {
            "base": report.base,
