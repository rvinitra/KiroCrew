...
Regression guard for a GPT 5.6 finding on PR #9413 (Windows support): every read
site in this file was widened from ``path.read_text()`` (platform-default decode)
to ``path.read_text(encoding="utf-8")`` so a Windows host cannot silently mis-decode
a crew record written with the platform's own default codec. That widening,
without more, introduces ``UnicodeDecodeError`` as a NEW way to fail on a file that
was written by any process using a different encoding -- a hand-edited file, or one
restored from a backup written by another version, both explicitly called out in
this module's own docstrings as things these readers must survive.
